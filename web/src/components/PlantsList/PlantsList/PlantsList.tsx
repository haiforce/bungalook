...
const PlantsList = ({ collection }: { collection: Collection }) => {
  const generateList = useCallback(() => {
    ...
    const result = collection.plants.map((item, index) => {
      return (
        <div className="col-xl-3 col-lg-4 col-md-6 wow fadeInUp" data-wow-delay={`${delayArray[index % 4]}s`}>
          <div className="product-item">
            <div className="position-relative bg-light overflow-hidden" style={{ maxHeight: "200px" }}>
              <img className="img-fluid w-100" src={item.media[0]?.value} alt="" />
          ...
          <div id="tab-1" className="tab-pane fade show p-0 active">
            <div className="row g-4">
              ...
              <div className="col-12 text-center wow fadeInUp" data-wow-delay="0.1s">
                <a className="btn btn-primary rounded-pill py-3 px-5" href="">Browse More Plants</a>
              </div>
            </div>
          </div>
        </div>
